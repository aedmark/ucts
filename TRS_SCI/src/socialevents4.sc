...
(procedure public (SocialEvent25)
	...
	(if(< Random(0 99) GLITCH_CHANCE_PCT)
		= glitchText "Rewrite your entire profile at\nmidnight out of spite."
	)
	= choice PrintChoices(
		...
		290
		glitchText
		"Reread the whole conversation\nlooking for the exact moment it went\nwrong." 0
		"Send one more message, lighter this\ntime, just in case they missed the\nfirst." 1
		"Unmatch and let it be exactly as\nunremarkable as it actually is." 2
	)
...
(procedure public (SocialEvent26)
	...
	= glitchText NULL
	(if(< Random(0 99) GLITCH_CHANCE_PCT)
		= glitchText "Have a friend text them from your\nphone 'as a joke.'"
	)
	= choice PrintChoices(
		...
		290
		glitchText
		"Decide you're clearly not that\ninterested and move on without\nsaying so." 0
		"Wait for them to text first so you\ndon't seem too eager." 1
		"Just text and ask if they want to\ngrab coffee this week." 2
	)
	(if(== choice GLITCH_CHOICE)
...
(procedure public (SocialEvent27)
	...
	= glitchText NULL
	(if(< Random(0 99) GLITCH_CHANCE_PCT)
		= glitchText "Look into whether you could\nrealistically drive three states in\none weekend."
	)
	= choice PrintChoices(
		...
		290
		glitchText
		"RSVP yes and figure out the money\nsomehow, some way you haven't found\nyet." 0
		"Don't respond to the invite for as\nlong as you possibly can." 1
		"Call your friend and just tell them\nthe truth about the money." 2
	)
	(if(== choice GLITCH_CHOICE)
...
(procedure public (SocialEvent28)
	...
	= glitchText NULL
	(if(< Random(0 99) GLITCH_CHANCE_PCT)
		= glitchText "Show up to the venue alone anyway,\non principle."
	)
	= choice PrintChoices(
		...
		290
		glitchText
		"Reply 'no worries at all!' and mean\nless than half of it." 0
		"Send a pointed text about the\npattern, right then, while you're\nstill annoyed." 1
		"Say it's fine tonight, and bring up\nthe pattern later when you're not\nstanding in your coat." 2
	)
	(if(== choice GLITCH_CHOICE)
...
(procedure public (SocialEvent29)
	...
	= glitchText NULL
	(if(< Random(0 99) GLITCH_CHANCE_PCT)
		= glitchText "Start giving them unsolicited advice\nback, immediately, at the same\nvolume."
	)
	= choice PrintChoices(
		...
		290
		glitchText
		"Nod, say 'that's a good point,' and\nabsorb advice you didn't ask for." 0
		"Cut them off and say you weren't\nactually asking for a solution." 1
		"Say gently that you just wanted to\nvent, not fix it right now. But\nthank you." 2
	)
	(if(== choice GLITCH_CHOICE)
...
(procedure public (SocialEvent30)
	...
	= glitchText NULL
	(if(< Random(0 99) GLITCH_CHANCE_PCT)
		= glitchText "Post something deliberately,\naggressively mundane in response."
	)
	= choice PrintChoices(
		...
		290
		glitchText
		"Keep scrolling well past the point\nit started feeling bad." 0
		"Like everything, generously, while\nfeeling worse with every tap." 1
		"Close the app and name, out loud,\nthat it's a highlight reel, not a\nlife." 2
	)
	(if(== choice GLITCH_CHOICE)
...
(procedure public (SocialEvent31)
	...
	= glitchText NULL
	(if(< Random(0 99) GLITCH_CHANCE_PCT)
		= glitchText "Try to convince everyone your\naccount got hacked."
	)
	= choice PrintChoices(
		...
		290
		glitchText
		"Stare at the message,\nunsent-but-sent, for a full minute\nbefore doing anything." 0
		"Immediately send eight apology\nmessages in a row." 1
		"Send one clear apology, own it, and\nstop typing." 2
	)
	(if(== choice GLITCH_CHOICE)
...
(procedure public (SocialEvent32)
	...
	= glitchText NULL
	(if(< Random(0 99) GLITCH_CHANCE_PCT)
		= glitchText "'Accidentally' plan something that\nconflicts with their next couple's\nouting."
	)
	= choice PrintChoices(
		...
		290
		glitchText
		"Say how much you love the new\ncouple, louder than you feel it." 0
		"Start finding reasons to skip the\ngroup hangouts for a while." 1
		"Tell your friend directly that you\nmiss the just-us version of hanging\nout." 2
	)
	(if(== choice GLITCH_CHOICE)
