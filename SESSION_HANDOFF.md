...
- **Game repo (git-tracked, this file lives here)**:
  ...
  - `TRS_SCI/art/` — the user's own working art/audio source folder
    (source images, a `.psd`, logo variants, MIDI candidates, etc.).
    **Ignore its contents entirely** — not part of the compiled game's
    source, managed manually by the user, not something to inventory,
    clean up, or treat as loose/unfamiliar state. Covered by
    `.gitignore` now (added alongside this note) so it stops showing up
    as untracked clutter in `git status`.
  - Any `.zip` directly under `TRS_SCI/` (seen so far as both
    `TRS_SCI.zip` and `Archive.zip` -- name varies) — just a zipped-up
    distribution build of the game, regenerated by the user as needed.
    Covered by `.gitignore`'s `TRS_SCI/*.zip` pattern rather than one
    exact filename, since it's been renamed at least once already.
...
15. ~~Revisit the Case Files viewer's browse-only scope cut~~ **Done,
    ...
    - **Current state**: `ShowCaseFiles()` (`CaseFiles.sc`) builds a
      ...
      `CaseFileTitle(index)` (`CaseFileTitles.sc`, Load/Dispose-scoped,
      ~107 cases) supplies each title via a switch. **Titles only --
      no descriptions exist anywhere accessible after the moment of
      discovery.** The actual flavor-text descriptions currently only
      ever get `Print()`ed once, live, inside each generated
      `PrintSurvivalEndingN()`/`PrintFailureEndingN()`
      (`endingsurvival0-8.sc`/`endingfailure0-2.sc`, from
      `tools/gen-endings.js`) or inline in `mechanisms.sc`'s unlock
      messages -- nowhere are they stored in a form the viewer could
      re-display later. Deliberately browse-only when built: "duplicating
      all 107 flavor-text strings a second time... isn't worth the extra
      resident heap for a browse-only reference screen."
    - **Why that calculus may have shifted, but the underlying caution
      hasn't gone away**: `CaseFiles.sc` itself is now Load/Dispose-scoped
      (this session's own fix) rather than permanently resident, so the
      *baseline* cost argument is weaker than when the scope cut was
      made. But this whole session was about discovering that SCI0's
      heap doesn't reliably give back what a script's `Load()` cost, even
      with a clean `DisposeScript()` -- a new script holding 107 full
      paragraph-length descriptions (much bigger than the existing
      title-only `CaseFileTitles.sc`) would need the exact same care:
      its own dedicated, Load/Dispose-scoped script (following
      `CaseFileTitles.sc`'s own precedent exactly), loaded only while
      the viewer is open.
    - **Real open design questions for next session, not decided here**:
      (1) *Data*: extend `tools/gen-endings.js` (and `mechanisms.sc`'s
      5 hardcoded unlock messages) to also emit a
      `CaseFileDescription(index)`-style accessor, or find another way
      to source the description text without re-typing 107 strings by
      hand. (2) *UI*: `ShowCaseFiles()`'s `DSelector` has only ever been
      used in pure browse-and-scroll mode (`state(1)`, no selection
      callback) -- showing a description on-demand needs some kind of
      "user picked an entry, now show more" interaction, which is
      genuinely new territory for this control in this codebase. The
      stock `SRDialog` (`syswindow.sc`, Save/Restore) is the one place
      in the template that DOES react to a `DSelector` selection
      meaningfully and is worth reading closely as a reference before
      inventing something. (3) *Categories*: the flat index scheme
      already has natural groupings baked in (`game.sh`: survival
      endings 0-71, failure endings 72-101, mechanisms 102-106, NG+ flag
      at 107 hidden from the viewer) -- whether "categories" means
      visually grouping the existing single list by these ranges (e.g.
