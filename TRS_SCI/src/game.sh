...
// Full paragraph-length descriptions, generated by
// tools/gen-casefile-descriptions.js from js/content-endings.js +
// js/content-mechanisms.js -- same Load/DisposeScript-scoped, only-needed-
// by-the-viewer idiom as CASEFILETITLES_SCRIPT (see its own header and
// SESSION_HANDOFF.md). Added when ShowCaseFiles() grew a "View" detail
// screen on top of its original browse-only titles list.
(define CASEFILEDESCRIPTIONS_SCRIPT	138)
...
// same 0-71/72-101/102-106 layout as the comment block above, just named
// so CaseFiles.sc doesn't hardcode raw index/count numbers for its three
// category screens.
