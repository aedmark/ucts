...
(procedure public (SelfEvent17)
	...
	(if(< Random(0 99) GLITCH_CHANCE_PCT)
		= glitchText "Chase them down the street to\nactually answer honestly."
	)
	= choice PrintChoices(
		...
		290
		glitchText
		"Say 'I'm fine!' faster than the\nquestion finished." 0
		"Deflect and change the subject\nimmediately." 1
		"Pause. Tell a small piece of the\ntruth." 2
	)
...
(procedure public (SelfEvent18)
	...
	= glitchText NULL
	(if(< Random(0 99) GLITCH_CHANCE_PCT)
		= glitchText "Call them right now and just say it,\nall of it, fast."
	)
	= choice PrintChoices(
		...
		290
		glitchText
		"Replay the moment, cataloging\nexactly why you didn't say it." 0
		"Decide it's better this way for\neveryone. Probably." 1
		"Note that the opening will come\nagain. It's not your only chance." 2
	)
	(if(== choice GLITCH_CHOICE)
...
(procedure public (SelfEvent19)
	...
	= glitchText NULL
	(if(< Random(0 99) GLITCH_CHANCE_PCT)
		= glitchText "Announce it to a stranger on the\nstreet, at volume."
	)
	= choice PrintChoices(
		...
		290
		glitchText
		"Immediately find the flaw to balance\nout the pride." 0
		"Feel the pride, quietly, and never\nmention it to anyone." 1
		"Say it out loud, once, to yourself.\n'I did that well.'" 2
	)
	(if(== choice GLITCH_CHOICE)
...
(procedure public (SelfEvent20)
	...
	= glitchText NULL
	(if(< Random(0 99) GLITCH_CHANCE_PCT)
		= glitchText "Draw it from memory, badly, in\ncrayon, right now."
	)
	= choice PrintChoices(
		...
		290
		glitchText
		"Lie there, awake, trying to force\nthe memory back." 0
		"Get up and immediately start the day\nlike nothing happened." 1
		"Let the feeling exist without the\nplot. You don't need the whole\nstory." 2
	)
	(if(== choice GLITCH_CHOICE)
...
(procedure public (SelfEvent21)
	...
	= glitchText NULL
	(if(< Random(0 99) GLITCH_CHANCE_PCT)
		= glitchText "Apologize to an inanimate object\ninstead. With real emotion."
	)
	= choice PrintChoices(
		...
		"Notice it happened and feel weird\nabout it for the rest of the day." 1
		"Notice the reflex. Don't perform an\napology for the apology." 2
	)
	(if(== choice GLITCH_CHOICE)
...
(procedure public (SelfEvent22)
	...
	= glitchText NULL
	(if(< Random(0 99) GLITCH_CHANCE_PCT)
		= glitchText "Post the most unhinged, honest thing\nyou can think of instead."
	)
	= choice PrintChoices(
		...
		290
		glitchText
		"Keep refreshing. The number hasn't\nproven anything yet, but the next\none might." 0
		"Post something calibrated to perform\nwell, just to be sure." 1
		"Put the phone down. The question\ndoesn't need an answer right now." 2
	)
	(if(== choice GLITCH_CHOICE)
...
(procedure public (SelfEvent23)
	...
	= glitchText NULL
	(if(< Random(0 99) GLITCH_CHANCE_PCT)
		= glitchText "Start an entirely new, deliberately\npointless streak instead."
	)
	= choice PrintChoices(
		...
		290
		glitchText
		"Berate yourself for the missed day,\nat length." 0
		"Decide the whole habit is ruined now\nand quietly stop." 1
		"Start again today. The forty-one\ndays still happened." 2
	)
	(if(== choice GLITCH_CHOICE)
...
(procedure public (SelfEvent24)
	...
	= glitchText NULL
	(if(< Random(0 99) GLITCH_CHANCE_PCT)
		= glitchText "Turn on the radio and sing along\nbadly through it."
	)
	= choice PrintChoices(
		...
		290
		glitchText
		"Stop it fast. Fix your face. Go\ninside like nothing happened." 0
		"Apologize to no one in an empty car\nfor crying at all." 1
		"Let it happen. Stay in the car until\nit passes on its own." 2
	)
	(if(== choice GLITCH_CHOICE)
